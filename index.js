...
async function parse2db (text) {
  ...
  const r = eachContent
    .slice(1, eachContent.length-1) // １つ目の要素（削除）
    .map((val) => val.split(']')) // 各情報のkey-valueを分ける
  let products = {}
  let user_info = {}
  const s = r.map((val) => {
    if (val[0].match(/】/)) { // '】'の有無で「注文商品情報」か「注文者情報」かを判別
      ...
      // 注文者情報
      switch (val[0]) {
        case 'お客様名': user_info['userName'] = val[1]
          break
        case 'メールアドレス': user_info['mail'] = val[1]
          break
        case '御社名(企業の方のみ)': user_info['companyName'] = val[1]
          break
        case 'お電話番号': user_info['tell'] = val[1]
          break
        case 'お届け先住所': user_info['address'] = val[1]
          break
        case 'お支払い方法': user_info['payment'] = val[1]
          break
        case '製品化してほしい車両パーツ(最大3つまで)': user_info['opinions'] = val[1]
        case 'その他': user_info['other'] = val[1]
          break
        default:
